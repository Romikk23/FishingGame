...
public class Save {
    ...
    public void makeSave() {
        try {
            ...
            String str;
            str = byteToString(Crypt.encrypt("time: " + time.hour + " " + time.minute));
            myWriter.write(str + "\n");

            str = byteToString(Crypt.encrypt("coor: " + player.worldX + " " + player.worldY));
            myWriter.write(str + "\n");

            str = byteToString(Crypt.encrypt("coin: " + player.coins));
            myWriter.write(str + "\n");

            for(int i = 1; i < player.inventory.size()+1; i++){
                str = byteToString(Crypt.encrypt("inve: " + player.inventory.get(i).id + " " + player.inventory.get(i).amount));
                myWriter.write(str + "\n");
            }
            myWriter.close();
